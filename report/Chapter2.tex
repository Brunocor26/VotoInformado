...
A aplicação foi desenvolvida para o sistema operativo Android \cite{AndroidDev}, utilizando a linguagem Java. A estrutura do projeto segue os padrões recomendados pela Google, organizando o código em componentes lógicos para facilitar a manutenção e escalabilidade.
...
O backend da aplicação, inicialmente projetado em Firebase, foi migrado para uma solução customizada para responder a requisitos de maior complexidade técnica e controlo. A arquitetura atual utiliza:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Nota de Migração}: A transição do Firebase Firestore para a API customizada foi motivada por uma sugestão pedagógica para aumentar a complexidade do projeto e demonstrar competências de desenvolvimento \textit{full-stack}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Retrofit/Gson} \cite{Retrofit} (Legado): Inicialmente utilizadas para parsing JSON, foram substituídas pela integração direta com o Firestore, mas fizeram parte do processo de desenvolvimento.
\end{itemize}