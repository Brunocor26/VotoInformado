...
Inicialmente, o projeto utilizava o Firebase Firestore para persistência de dados diretamente na aplicação móvel. No entanto, seguindo uma \textbf{sugestão pedagógica do docente} para aumentar a complexidade técnica do projeto e permitir uma solução mais personalizada, migrou-se para uma arquitetura com uma **API REST intermédia**.
...
Foi implementada a funcionalidade de eliminação de petições através do método HTTP \texttt{DELETE}. O endpoint \texttt{/api/petitions/:id} permite remover documentos da coleção \texttt{petitions} no MongoDB, garantindo a integridade dos dados. A aplicação Android expõe esta funcionalidade através da classe utilitária \texttt{DatabaseHelper}.
...
Foi detetado que o upload de imagens falhava em certos dispositivos ou quando a imagem provinha de fontes específicas (como Google Photos), resultando em falhas na obtenção do tipo de ficheiro (MIME type) ou do nome original.
...
\begin{itemize}
    ...
    \item \textbf{Tipos de Ficheiro}: Implementou-se um mecanismo de \textit{fallback} que assume o tipo JPEG caso o sistema não consiga determinar o tipo MIME, prevenindo \texttt{NullPointerExceptions}.
\end{itemize}

\subsection{Estabilidade na Votação de Debates}
A funcionalidade de votação em debates apresentava instabilidade (\textit{crashes}) quando os dados de votos continham inconsistências, especificamente quando o ID do candidato votado era nulo (dados corrompidos ou legados).

\textbf{Solução}: Adicionaram-se verificações de nulidade explícitas no método \texttt{processVotes} da \texttt{DebateVoteActivity}. O algoritmo ignora agora votos malformados em vez de interromper a execução, garantindo que a aplicação continua funcional mesmo com dados imperfeitos.
