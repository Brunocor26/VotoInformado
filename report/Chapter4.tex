...
\section{Integração de Mapas} \label{sec:impl_maps}
Para a visualização geográfica, optou-se pela integração do \textbf{Google Maps SDK for Android}.
\begin{itemize}
    \item \textbf{Configuração}: A chave de API (\texttt{API\_KEY}) foi configurada no \texttt{AndroidManifest.xml}, permitindo o acesso aos serviços de mapas da Google.
    \item \textbf{Implementação}: O \texttt{HomeFragment} implementa a interface \texttt{OnMapReadyCallback}. O mapa é carregado assincronamente através de um \texttt{SupportMapFragment}, e quando pronto (\texttt{onMapReady}), a câmara é posicionada nas coordenadas predefinidas (e.g., Lisboa), oferecendo uma base para futura visualização de eventos georreferenciados.
\end{itemize}
